import {
    ChangeDetectionStrategy, ChangeDetectorRef, Component, EventEmitter, HostBinding, Input, NgZone,
...
export class NotificationItemComponent implements OnInit, OnDestroy {
  ...
  @Output() destroy = new EventEmitter<Notification>();
  ...
  tryClear(notification: Notification) {
    if (this.notification.closable)
      this.destroy.emit(notification);
  }
  ...
  private instance = () => {
    ...
    if (this.count++ === this.steps) {
      this.destroy.emit(this.notification);
    }
